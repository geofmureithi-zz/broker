...
use portal::{reject, Filter, Rejection, Reply};
...
#[tokio::main]
async fn main() {
    let math = portal::path!("math" / u16)
        .and(div_by())
        .map(|num: u16, denom: NonZeroU16| {
            portal::reply::json(&Math {
                op: format!("{} / {}", num, denom),
                output: num / denom.get(),
            })
        });

    let routes = portal::get().and(math).recover(handle_rejection);
...
fn div_by() -> impl Filter<Extract = (NonZeroU16,), Error = Rejection> + Copy {
    portal::header::<u16>("div-by").and_then(|n: u16| {
        async move {
...
async fn handle_rejection(err: Rejection) -> Result<impl Reply, Infallible> {
    ...
    } else if let Some(_) = err.find::<portal::reject::MethodNotAllowed>() {
    ...
    let json = portal::reply::json(&ErrorMessage {
        code: code.as_u16(),
        message: message.into(),
    });

    Ok(portal::reply::with_status(json, code))
}
