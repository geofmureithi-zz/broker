...
async fn main() {
    ...
    // GET /hi
    let hi = portal::path("hi").map(|| "Hello, World!");

    // How about multiple segments? First, we could use the `path!` macro:
    //
    // GET /hello/from/portal
    let hello_from_portal = portal::path!("hello" / "from" / "portal").map(|| "Hello from portal!");

    // Fine, but how do I handle parameters in paths?
    //
    // GET /sum/:u32/:u32
    let sum = portal::path!("sum" / u32 / u32).map(|a, b| format!("{} + {} = {}", a, b, a + b));

    // Any type that implements FromStr can be used, and in any order:
    //
    // GET /:u16/times/:u16
    let times =
        portal::path!(u16 / "times" / u16).map(|a, b| format!("{} times {} = {}", a, b, a * b));
    ...
    // GET /math/sum/:u32/:u32
    // GET /math/:u16/times/:u16
    let math = portal::path("math").and(sum.or(times));

    // We can use the end() filter to match a shorter path
    let help = portal::path("math")
    ...
    let routes = portal::get().and(hi.or(hello_from_portal).or(bye).or(math).or(sum).or(times));
