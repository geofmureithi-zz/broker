...
use tokio::sync::mpsc;
use portal::ws::{Message, WebSocket};
use portal::Filter;
...
///
/// - Key is their id
/// - Value is a sender of `portal::ws::Message`
type Users = Arc<Mutex<HashMap<usize, mpsc::UnboundedSender<Result<Message, portal::Error>>>>>;
...
async fn main() {
    ...
    let chat = portal::path("chat")
        // The `ws()` filter will prepare Websocket handshake...
        .and(portal::ws())
        .and(users)
        .map(|ws: portal::ws::Ws, users| {
            // This will call our function if the handshake succeeds.
            ws.on_upgrade(move |socket| user_connected(socket, users).map(|result| result.unwrap()))
        });

    // GET / -> index html
    let index = portal::path::end().map(|| portal::reply::html(INDEX_HTML));
