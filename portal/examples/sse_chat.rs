...
#[derive(Debug)]
struct NotUtf8;
impl portal::reject::Reject for NotUtf8 {}
...
async fn main() {
    ...
    let users = portal::any().map(move || users.clone());

    // POST /chat -> send message
    let chat_send = portal::path("chat")
        .and(portal::post())
        .and(portal::path::param::<usize>())
        .and(portal::body::content_length_limit(500))
        .and(portal::body::bytes().and_then(|body: bytes::Bytes| {
            async move {
                std::str::from_utf8(&body)
                    .map(String::from)
                    .map_err(|_e| portal::reject::custom(NotUtf8))
            }
        }))
        .and(users.clone())
        .map(|my_id, msg, users| {
    ...
    let chat_recv = portal::path("chat").and(portal::get()).and(users).map(|users| {
        // reply using server-sent events
        let stream = user_connected(users);
        portal::sse::reply(portal::sse::keep_alive().stream(stream))
    });

    // GET / -> index html
    let index = portal::path::end().map(|| {
        portal::http::Response::builder()
...
fn user_connected(
    users: Users,
) -> impl Stream<Item = Result<impl ServerSentEvent + Send + 'static, portal::Error>> + Send + 'static
{
    ...
    rx.map(|msg| match msg {
        Message::UserId(my_id) => Ok((portal::sse::event("user"), portal::sse::data(my_id)).into_a()),
        Message::Reply(reply) => Ok(portal::sse::data(reply).into_b()),
    })
}
