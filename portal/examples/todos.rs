...
async fn main() {
    ...
    let routes = api.with(portal::log("todos"));
    // Start up the server...
...
mod filters {
    ...
    pub fn todos(
        db: Db,
    ) -> impl Filter<Extract = impl portal::Reply, Error = portal::Rejection> + Clone {
        todos_list(db.clone())
    ...
    pub fn todos_delete(
        ...
        let admin_only = portal::header::exact("authorization", "Bearer admin");
    ...
    fn with_db(db: Db) -> impl Filter<Extract = (Db,), Error = std::convert::Infallible> + Clone {
        portal::any().map(move || db.clone())
    }

    fn json_body() -> impl Filter<Extract = (Todo,), Error = portal::Rejection> + Clone {
        // When accepting a body, we want a JSON body
        // (and to reject huge payloads)...
        portal::body::content_length_limit(1024 * 16).and(portal::body::json())
    }
}
...
mod handlers {
    ...
    use portal::http::StatusCode;

    pub async fn list_todos(opts: ListOptions, db: Db) -> Result<impl portal::Reply, Infallible> {
        ...
        let todos: Vec<Todo> = todos
            ...
            .collect();
        Ok(portal::reply::json(&todos))
    }

    pub async fn create_todo(create: Todo, db: Db) -> Result<impl portal::Reply, Infallible> {
    ...
    pub async fn update_todo(
        ...
        update: Todo,
        db: Db,
    ) -> Result<impl portal::Reply, Infallible> {
    ...
    pub async fn delete_todo(id: u64, db: Db) -> Result<impl portal::Reply, Infallible> {
