...
//! use portal::Filter;
//!
//! fn sum() -> impl Filter<Extract = (u32,), Error = portal::Rejection> + Copy {
//!     portal::path::param()
...
//! }
//!
//! fn math() -> impl Filter<Extract = (String,), Error = portal::Rejection> + Copy {
//!     portal::post()
//!         .and(sum())
//!         .map(|z: u32| {
//!             format!("Sum = {}", z)
...
//! #[test]
//! fn test_sum() {
//! #    let sum = || portal::any().map(|| 3);
//!     let filter = sum();
//!
//!     // Execute `sum` and get the `Extract` back.
//!     let value = portal::test::request()
//!         .path("/1/2")
//!         .filter(&filter)
//!         .unwrap();
//!     assert_eq!(value, 3);
...
//! #[test]
//! fn test_math() {
//! #    let math = || portal::any().map(portal::reply);
//!     let filter = sum();
//!
