...
//!
//! ```
//! use portal::{Filter, http::Response};
//!
//! // Returns an empty `200 OK` response.
//! let empty_200 = portal::any().map(portal::reply);
//!
//! // Returns a `200 OK` response with custom header and body.
//! let custom = portal::any().map(|| {
//!     Response::builder()
...
//! let routes = portal::get().and(empty_200)
//!     .or(portal::post().and(custom));
//! ```
...
/// // `[1, 3, 7, 13]`
/// let route = portal::path("ids")
///     .map(|| {
///         let our_ids = vec![1, 3, 7, 13];
///         portal::reply::json(&our_ids)
///     });
/// ```
///
/// # Note
...
impl fmt::Display for ReplyJsonError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str("portal::reply::json() failed")
    }
}
...
/// }
///
/// impl portal::Reply for Message {
///     fn into_response(self) -> portal::reply::Response {
...
/// }
///
/// let route = portal::any().map(handler);
/// ```
...
///     .map(portal::reply)
///     .map(|reply| {
///         portal::reply::with_status(reply, portal::http::StatusCode::CREATED)
///     });
/// ```
...
/// Wrap an `impl Reply` to change its `StatusCode`.
///
/// Returned by `portal::reply::with_status`.
#[derive(Debug)]
...
///     .map(portal::reply)
///     .map(|reply| {
///         portal::reply::with_header(reply, "server", "portal")
///     });
/// ```
...
///
/// Returned by `portal::reply::with_header`.
#[derive(Debug)]
