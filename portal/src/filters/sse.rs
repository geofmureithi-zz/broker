...
//! use portal::{Filter, sse::ServerSentEvent};
//! use futures::{stream::iter, Stream};
//!
//! fn sse_events() -> impl Stream<Item = Result<impl ServerSentEvent, Infallible>> {
//!     iter(vec![
//!         Ok(portal::sse::data("unnamed event").into_a()),
//!         Ok((
//!             portal::sse::event("chat"),
//!             portal::sse::data("chat message"),
//!         ).into_a().into_b()),
//!         Ok((
//!             portal::sse::id(13),
//!             portal::sse::event("chat"),
//!             portal::sse::data("other chat message\nwith next line"),
//!             portal::sse::retry(Duration::from_millis(5000)),
//!         ).into_b().into_b()),
//!     ])
//! }
//!
//! let app = portal::path("push-notifications")
//!     .and(portal::get())
//!     .map(|| {
//!         portal::sse::reply(portal::sse::keep_alive().stream(sse_events()))
//!     });
//! ```
//!
...
///
/// ```
/// let app = portal::sse::last_event_id::<u32>();
///
...
///             Ok(portal::sse::data("payload").boxed()),
///             // Named event with ID and retry timeout
///             Ok((
///                 portal::sse::data("other message\nwith next line"),
///                 portal::sse::event("chat"),
///                 portal::sse::id(1),
///                 portal::sse::retry(Duration::from_millis(15000))
///             ).boxed()),
...
///
/// async {
///     let app = portal::path("sse").and(portal::get()).map(|| {
    ...
    S::Error: StdError + Send + Sync + 'static,
{
    ...
    fn into_response(self) -> Response  {
        let body_stream = self
            ...
            .into_stream()
            .and_then(|event| {
                future::ready(SseWrapper::format(&event))
            });
...
#[doc(hidden)]
#[deprecated(note = "use portal::see:keep_alive() instead")]
pub fn keep<S>(
    event_stream: S,
...
/// use portal::{Filter, Stream, sse::ServerSentEvent};
///
...
///             let stream = portal::sse::keep_alive()
///                 .interval(Duration::from_secs(5))
///                 .text("thump".to_string())
///                 .stream(event_stream);
///             portal::sse::reply(stream)
///         });
/// }
/// ```
///
