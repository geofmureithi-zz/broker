...
//! # Routing
//!
//! Routing in portal is simple yet powerful.
//!
...
//! ```
//! # use portal::Filter;
//! // GET /hello/from/portal
//! let hello_from_portal = portal::path!("hello" / "from" / "portal").map(|| {
//!     "Hello from portal!"
//! });
//! ```
//!
...
//! # use portal::Filter;
//! // GET /sum/:u32/:u32
//! let sum = portal::path!("sum" / u32 / u32).map(|a, b| {
//!     format!("{} + {} = {}", a, b, a + b)
//! });
//! ```
//!
...
//! # use portal::Filter;
//! // GET /:u16/times/:u16
//! let times = portal::path!(u16 / "times" / u16).map(|a, b| {
...
//! ```
//! # use portal::Filter;
//! # let sum = portal::any().map(portal::reply);
//! # let times = sum.clone();
//! // GET /math/sum/:u32/:u32
//! // GET /math/:u16/times/:u16
//! let math = portal::path("math");
...
//! ```
//! # use portal::Filter;
//! # let sum = portal::path("sum");
//! # let times = portal::path("times");
//! // GET /math/sum/:u32/:u32
//! // GET /math/:u16/times/:u16
//! let math = portal::path("math")
//!     .and(sum.or(times));
...
//! ```
//! # use portal::Filter;
//! # let hi = portal::path("hi");
//! # let hello_from_portal = hi.clone();
//! # let bye = hi.clone();
//! # let math = hi.clone();
//! // GET /hi
//! // GET /hello/from/portal
...
/// use portal::Filter;
///
/// let route = portal::path("foo")
///     .and(portal::path::tail())
///     .map(|tail| {
...
///
/// ```
/// use portal::{Filter, path::FullPath};
/// use std::{collections::HashMap, sync::{Arc, Mutex}};
///
/// let counts = Arc::new(Mutex::new(HashMap::new()));
/// let access_counter = portal::path::full()
