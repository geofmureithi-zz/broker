...
///
/// ```compile_fail
/// fn with(r: portal::Rejection) {
///     let _wat = portal::reject::custom(r);
/// }
/// ```
...
///
/// ```
/// use portal::{Filter, reject::Reject};
///
/// #[derive(Debug)]
/// struct RateLimited;
///
...
///
/// let route = portal::any().and_then(|| async {
///     Err::<(), _>(portal::reject::custom(RateLimited))
/// });
/// ```
...
impl Rejection {
    ...
    /// struct Nope;
    ///
    /// impl portal::reject::Reject for Nope {}
    ///
    /// let reject = portal::reject::custom(Nope);
    ///
    ...
    /// Returns true if this Rejection was made via `portal::reject::not_found`.
    ///
    /// # Example
    ///
    /// ```
    /// let rejection = portal::reject();
    ///
...
mod sealed {
    ...
    pub trait CombineRejection<E>: Send + Sized {
        ...
        /// # For example:
        ///
        /// `portal::any().and(portal::path("foo"))` has the following steps:
        ///
        /// 1. Since this is `and`, only **one** of the rejections will occur,
        ///    and as soon as it does, it will be returned.
        /// 2. `portal::any()` rejects with `Never`. So, it will never return `Never`.
        /// 3. `portal::path()` rejects with `Rejection`. It may return `Rejection`.
        ///
