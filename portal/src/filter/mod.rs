...
pub trait Filter: FilterBase {
    ...
    /// portal::path::param::<u32>()
    ///     .or(portal::path::param::<SocketAddr>());
    /// ```
    ...
    ///
    /// // Validate after `/:id`
    /// portal::path::param().and_then(|id: u64| async move {
    ///     if id != 0 {
    ///         Ok(format!("Hello #{}", id))
    ///     } else {
    ///         Err(portal::reject::not_found())
    ///     }
    /// });
    /// ```
    ...
    ///
    /// [ex]: https://github.com/seanmonstar/portal/blob/master/examples/rejections.rs
    ...
    /// use portal::Filter;
    ///
    /// let client_ip = portal::header("x-real-ip")
    ///     .or(portal::header("x-forwarded-for"))
    ///     .unify()
    ///     .map(|ip: SocketAddr| {
    ...
    /// but just `()`, since portal will wrap it up into a `((),)`.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// let route = route.with(portal::log("example"));
    /// ```
    ...
    /// use portal::Filter;
    ///
    /// fn impl_reply() -> portal::filters::BoxedFilter<(impl portal::Reply,)> {
    ///     portal::any()
    ///         .map(portal::reply)
    ///         .boxed()
    /// }
    ///
    /// fn named_i32() -> portal::filters::BoxedFilter<(i32,)> {
    ///     portal::path::param::<i32>()
    ///         .boxed()
    /// }
    ///
    /// fn named_and() -> portal::filters::BoxedFilter<(i32, String)> {
    ///     portal::path::param::<i32>()
    ///         .and(portal::header::<String>("host"))
    ///         .boxed()
    /// }
    /// ```
