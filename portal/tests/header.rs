...
async fn exact() {
    ...
    let host = portal::header::exact("host", "localhost");

    let req = portal::test::request().header("host", "localhost");
    ...
    let req = portal::test::request().header("host", "hyper.rs");
...
async fn exact_rejections() {
    ...
    let host = portal::header::exact("host", "localhost").map(portal::reply);
...
async fn optional() {
    ...
    let con_len = portal::header::optional::<u64>("content-length");

    let val = portal::test::request()
        .filter(&con_len)
        .await
